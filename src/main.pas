...
type
  TF_Main = class(TForm)
    ...
    procedure PageControlCloseButtonMouseUp(Sender: TObject;
      Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure PageControlDragDrop(Sender, Source: TObject; X,
      Y: Integer);
...
var host:string;
    port:Word;
begin
 F_Main.SB_Main.Panels[0].Text := 'Pøipojování ...';
...
begin
 if (PanelTCPClient.status <> TPanelConnectionStatus.opened) then
  begin
   Application.MessageBox(PChar('Program není pøipojen k serveru, nelze žádat o lokomotivu.'+#13#10+'Pøipojit k serveru se mùžete v oknì nastavení.'), 'Nelze žádat o lokomotivu', MB_OK OR MB_ICONWARNING);
   Exit();
  end;
...
procedure TF_Main.PageControlDragDrop(Sender, Source: TObject; X,
  Y: Integer);
const
...
begin
   ...
   for j := 0 to Self.PC_Main.PageCount - 1 do
   begin
