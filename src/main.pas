...
begin
 if (PanelTCPClient.status <> TPanelConnectionStatus.opened) then
  begin
   Application.MessageBox(PChar('Program není připojen k serveru, nelze žádat o lokomotivu.'+#13#10+'Připojit k serveru se můžete v okně nastavení.'), 'Nelze žádat o lokomotivu', MB_OK OR MB_ICONWARNING);
   Exit();
  end;
